...
class Kurisu(commands.Bot):
    ...
    async def load_roles(self):
        roles = ['Staff', 'Owner', 'Probation', "No-U", "NSFW Access", "User", "Announcement", "VoiceChat", "Server Booster", '🍰', "Admin", "Moderator", "No-Help", "help-mute", "meta-mute", "appeal-mute"]
...
async def startup():
    ...
    async with asyncpg.create_pool(DATABASE_URL, min_size=20, max_size=20) as pool:
        ...
        bot = Kurisu(command_prefix=['.', '!'], description=f"Aurisu, the bot for {self.guild.name}!", commit=commit,
